...
recaptchaSiteKey = '6Lf0JssSAAAAAGI-v6jS5ZOMEZheVYGpGlUeT3HK';
...
googleMapAPIKey = 'AIzaSyBnNN9UyYQpvWyUNvS5P8MGd6EVMw9fHNE';
...
jQuery(document).ready(function () {
    ...
    if (jQuery('.contact-form-block').length) {
        jQuery('.contact-form-block form').each(function (index) {
            ...
            jQuery('#gfort-contact-form-block-' + index).submit(function () {
                ...
                jQuery.post('/wf/send/contact/?c=1091', formValues, function (response) {
    ...
    Contact Form
    ========================================================================= */
    if (jQuery('.report-free-form-block').length) {
        jQuery('.report-free-form-block form').each(function (index) {

            jQuery(this).attr('id', 'gfort-report-free-form-block-' + index);

            jQuery('#gfort-report-free-form-block-' + index).submit(function () {
                ...
                jQuery.post('/wf/send/reportfree/?c=1091', formValues, function (response) {
                    ...
                    if (response.match('error-web-site') !== null) {
                        el.find('.contact-web-site').next().addClass('error');
                        el.find('button').removeClass('add-spin');
                    }
                    ...
                    /* Keywords
                    ----------------------------------------------------- */
                    if (response.match('error-keywords') !== null) {
                        el.find('.contact-keywords').next().addClass('error');
                        el.find('button').removeClass('add-spin');
                    }
    ...
    Tools Error Form
    ========================================================================= */
    if (jQuery('.tools-error-form-block').length) {
        jQuery('.tools-error-form-block form').each(function (index) {

            jQuery(this).attr('id', 'gfort-tools-error-form-block-' + index);

            jQuery('#gfort-tools-error-form-block-' + index).submit(function () {
                ...
                jQuery.post('/wf/send/errortoolmessage/?c=1091', formValues, function (response) {
    ...
    function gfortSelectfn() {
        jQuery('select').each(function (index) {
            ...
            if (jQuery(this).hasClass('tool')) return;
    ...
    function mapMarkersfn(currentMapIndex) {
        ...
        jQuery.ajax({
            ...
            success: function (response) {
                ...
                if (response != undefined && response.locations != undefined) {
...
var waitingDialog = waitingDialog || (function ($) {
    ...
    return {
        ...
        show: function (message, options) {
            ...
            $dialog.find('h4').html("<i class='fa fa-cog fa-spin'></i> " + message);
            // Adding callbacks
